...
class getBaiduQianxi:
    is_the_first_time = True
    global_area_code = ''
    def create_xlsx(self, json_path, date, scale_choice):
    ...
    def process_scale_choice(self, scale_choice):
        base_url = ""
        scale_choice = int(scale_choice)
        if scale_choice == 1:
    ...
    def process_date_choice(self, date_choice):
        tiny_list = []
    ...
    def process_direction_choice(self, direction_choice):
    ...
    def process_area_choice(self, area_choice):
        ...
        if area_choice == 1:
            ...
            area_code = input('请输入区域代码：')
        return area_code


    def generate_date_list(self, date_se_list:list):
    ...
    def create_directory(self, area_choice, date_choice, direction_choice, scale_choice):
        area_choice = int(area_choice)
    ...
    def init_request(self, scale_choice, date, direction_choice, area_choice):
        is_the_first_time = True
        base_url = self.process_scale_choice(scale_choice=scale_choice)
        direction = self.process_direction_choice(direction_choice)
        if self.is_the_first_time:
            self.global_area_code = self.process_area_choice(area_choice)
            self.is_the_first_time = False
        dt_scale = 'dt_scale'
        if int(self.global_area_code[-3]) == 0:
            dt_scale = 'province'
        elif not int(self.global_area_code[-3]) == 0:
            dt_scale = 'city'
        para = {
            # 如果主体为省，则需要改dt
            "dt": dt_scale,
            "id": self.global_area_code,
            "type": direction,
            "date": date
        }
